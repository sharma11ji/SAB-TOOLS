...
export async function saveCuttingList(uid,projectId,data,id){guard();const ref=id?doc(db,"users",uid,"projects",projectId,"cuttingLists",id):doc(col(uid,"projects",projectId+"/cuttingLists"));await setDoc(ref,{...clean(data),updatedAt:serverTimestamp(),...(id?{}:{createdAt:serverTimestamp()})},{merge:true});return ref.id}
